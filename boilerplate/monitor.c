    ...
    * --------------------------------------------------------------- */
    static void timer_callback(struct timer_list *t)
    {
        ...
        * ============================================================== */
        ...
        /*
        * Use a mutex:
        * - ioctl handler uses copy_from_user + kmalloc (can sleep)
        * - timer callback here does get_rss_bytes() which can take locks and sleep
        * So mutex is correct and avoids spinlock-in-sleep bugs.
        */
        mutex_lock(&monitored_lock);
        list_for_each_entry_safe(e, tmp, &monitored_list, list) {
            long rss = get_rss_bytes(e->pid);

            /* Process exited or PID not found: stop tracking */
            if (rss < 0) {
                list_del(&e->list);
                kfree(e);
                continue;
            }
            /* Hard limit: kill and remove entry */
    ...
    * --------------------------------------------------------------- */
    static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
    {
        ...
        if (cmd == MONITOR_REGISTER) {
            ...
            * ============================================================== */
            ...
            if (req.hard_limit_bytes > 0 &&
                ...
                printk(KERN_WARNING
                    "[container_monitor] Invalid limits: soft > hard (container=%s pid=%d)\n",
            ...
            e->pid = req.pid;
            strscpy(e->container_id, req.container_id, sizeof(e->container_id));
            e->soft_limit_bytes = req.soft_limit_bytes;
            e->hard_limit_bytes = req.hard_limit_bytes;
            e->soft_warned = false;
            INIT_LIST_HEAD(&e->list);
            mutex_lock(&monitored_lock);
            /* If same PID already registered, replace it (no duplicates). */
            struct monitored_entry *it2, *it_tmp;
            list_for_each_entry_safe(it2, it_tmp, &monitored_list, list) {
                if (it2->pid == req.pid) {
                    list_del(&it2->list);
                    kfree(it2);
                    break;
        ...
        * ============================================================== */
        ...
        list_for_each_entry_safe(e, tmp, &monitored_list, list) {
            /*
            * Match on pid AND container_id to avoid killing the wrong entry
            * if PIDs get reused. (Best-effort: container_id is stable.)
            */
            if (e->pid == req.pid &&
                strncmp(e->container_id, req.container_id, MONITOR_NAME_LEN) == 0) {
                list_del(&e->list);
                kfree(e);
                mutex_unlock(&monitored_lock);
                return 0;
